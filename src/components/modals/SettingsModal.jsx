...
import { X, Building2, Server, Lock, Plus, Trash2, ArrowDown, Upload, RotateCcw } from 'lucide-react';
...
export const DEFAULT_COLUMNS = [
    ...
    { id: '2', header: 'EMP CODE', systemField: 'Emp Code', showOnDashboard: true },
    { id: '3', header: 'EMPLOYEE NAME', systemField: 'Name', showOnDashboard: true },
    { id: '4', header: 'ACCOUNT NUMBER', systemField: 'Bank Account', showOnDashboard: false },
    ...
    { id: '8', header: 'DESIGNATION', systemField: 'Designation', showOnDashboard: true },
    { id: '9', header: 'DEPARTMENT', systemField: 'Department', showOnDashboard: true },
    { id: '10', header: 'DOJ', systemField: 'DOJ', showOnDashboard: true },
    { id: '11', header: 'LOCATION', systemField: 'Location', showOnDashboard: true },
    { id: '12', header: 'EMAIL', systemField: 'Email', showOnDashboard: true },
    { id: '13', header: 'ESI IP NO.', systemField: 'ESIC No', showOnDashboard: false },
    ...
    { id: '19', header: 'CONV ALW', systemField: 'CONV ALW', showOnDashboard: false },
    { id: '20', header: 'MED ALW', systemField: 'MED ALW', showOnDashboard: false },
    { id: '21', header: 'OTHER ALW', systemField: 'OTHER ALW', showOnDashboard: false },
    { id: '22', header: 'DIST ALW', systemField: 'DIST ALW', showOnDashboard: false },
    { id: '23', header: 'ARREARS', systemField: 'ARREARS', showOnDashboard: false },
    { id: '24', header: 'GROSS SALARY', systemField: '', showOnDashboard: false },
    { id: '25', header: 'PRESENT DAYS', systemField: 'Days Paid', showOnDashboard: false },
    // New Defaults matching User's Excel
    { id: '26', header: 'EARNED SALARY', systemField: 'GROSS SALARY', showOnDashboard: false },
    { id: '27', header: 'VPF', systemField: 'VPF', showOnDashboard: false },
    { id: '28', header: 'STAFF LOAN', systemField: 'STAFF LOAN', showOnDashboard: false },
    { id: '29', header: 'SAL ADV', systemField: 'SAL ADV', showOnDashboard: false },
    { id: '30', header: 'LWF', systemField: 'LWF', showOnDashboard: false },
    { id: '31', header: 'MOBILE/ OTHER DED', systemField: 'MOBILE_DED', showOnDashboard: false },
    { id: '32', header: 'OTHER DEDUCTION', systemField: 'OTHER_DED', showOnDashboard: false },
    { id: '33', header: 'MEDICAL INSURANCE', systemField: 'MEDICAL_INS', showOnDashboard: false },
    { id: '34', header: 'TTL DED', systemField: 'TTL DED', showOnDashboard: false },
    { id: '35', header: 'NET SALARY', systemField: 'NET SALARY', showOnDashboard: true },
];
...
const CustomSelect = ({ value, onChange, options, placeholder = "Select..." }) => {
    ...
    useEffect(() => {
        ...
        const handleScroll = (event) => {
            ...
            if (event.target.classList?.contains?.('custom-select-dropdown') || event.target.closest?.('.custom-select-dropdown')) {
                return;
            }
            setIsOpen(false);
        };
    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            <div
                className={`w-full px-2 py-1 text-sm border rounded outline-none cursor-pointer flex items-center justify-between bg-white ${value ? 'border-indigo-300 text-indigo-700 bg-indigo-50/50' : 'border-slate-200 text-slate-600'
                    }`}
                onClick={handleToggle}
            >
                <span className="truncate">{selectedOption ? selectedOption.label : placeholder}</span>
                <ArrowDown size={12} className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} opacity-50`} />
            </div>
            ...
            {isOpen && createPortal(
                ...
                >
                    ...
                    {options.map((option) => (
                        <div
                            key={option.value}
                            className={`px-3 py-2 text-sm cursor-pointer hover:bg-indigo-50 flex items-center justify-between transition-colors ${option.disabled ? 'opacity-50 cursor-not-allowed bg-slate-50' : ''
                        ...
                        >
                            <span className="truncate pr-2">{option.prefix}{option.label}{option.suffix}</span>
                            {option.value === value && <span className="text-indigo-600 shrink-0">✓</span>}
                        </div>
                    ))}
                </div>,
                document.body
...
export default function SettingsModal({
    ...
    const handleSampleUpload = (e) => {
        ...
        reader.onload = (evt) => {
            ...
            if (jsonData.length <= headerRowIndex) {
                showToast('File contains fewer rows than the specified Title Rows to Skip.', 'error');
                return;
            }
            ...
            const newMapping = headers.map((header, index) => {
                ...
                const existing = settingsForm.columnMapping.find(c => c.header === cleanHeader);
    ...
    const handleResetMapping = () => {
        setConfirmModal({
            isOpen: true,
            title: 'Reset Mapping',
            message: 'Are you sure you want to reset the column mapping to defaults? This will clear all custom indices and mappings.',
            type: 'warning',
            onConfirm: () => {
                setSettingsForm(prev => ({
                    ...prev,
    ...
    const handleSave = () => {
        ...
        SYSTEM_FIELDS.forEach(field => {
            if (field.required) {
                ...
                const isMapped = settingsForm.columnMapping.some(col => col.systemField === field.value);
                if (!isMapped) {
        ...
        if (missingFields.length > 0) {
            showToast(`Missing mandatory fields: ${missingFields.join(', ')}`, 'error');
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" style={{ '--brand-color': settingsForm.brandColor || '#4f46e5' }}>
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex border-b border-slate-200 px-6 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                    <button
                        ...
                        className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${settingsTab === 'account'
                    ...
                    <button
                        ...
                        className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${settingsTab === 'branding'
                            ? 'border-(--brand-color) text-(--brand-color)'
                            : 'border-transparent text-slate-500 hover:text-slate-700'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${settingsTab === 'smtp'
                            ? 'border-(--brand-color) text-(--brand-color)'
                            : 'border-transparent text-slate-500 hover:text-slate-700'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${settingsTab === 'data_format'
                            ? 'border-(--brand-color) text-(--brand-color)'
                            : 'border-transparent text-slate-500 hover:text-slate-700'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${settingsTab === 'security'
                            ? 'border-(--brand-color) text-(--brand-color)'
                            : 'border-transparent text-slate-500 hover:text-slate-700'
                            }`}
                    >
                ...
                <div className="p-6 min-h-[300px]">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                <div className="flex items-center justify-between mb-2">
                                    <label className="text-xs font-medium text-slate-700">Email Status</label>
                                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${user?.emailVerified ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                        {user?.emailVerified ? 'Verified' : 'Unverified'}
                                    </span>
                                </div>
                                <p className="text-sm text-slate-800 font-medium mb-3">{user?.email}</p>
                                {!user?.emailVerified && (
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                if (e.code === 'auth/too-many-requests' || e.message?.includes('TOO_MANY_ATTEMPTS_TRY_LATER')) {
                                                    showToast('Too many attempts. Please check your inbox or try again later.', 'error');
                                                } else {
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-medium text-slate-700 mb-1">User Name</label>
                                        <input
                                            type="text"
                        ...
                        <div className="space-y-6">
                            <div className="flex items-start gap-6">
                                <div className="flex-1">
                                    <label className="block text-xs font-medium text-slate-700 mb-2">Company Logo</label>
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 bg-slate-100 rounded-lg flex items-center justify-center overflow-hidden border">
                                            {settingsForm.logoPreview ? (
                                                <>
                                                    ...
                                                    <img src={settingsForm.logoPreview} alt="Preview" className="w-full h-full object-contain" />
                                                </>
                                            ) : (
                                        ...
                                        <label className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-medium rounded-lg cursor-pointer transition shadow-sm">
                                            Change Logo
                                            <input type="file" accept="image/*" onChange={handleSettingsLogoUpload} className="hidden" />
                                        </label>
                                    </div>
                                </div>
                                <div className="flex-1">
                                    <label className="block text-xs font-medium text-slate-700 mb-2">Brand Color</label>
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <span className="text-sm font-mono text-slate-600">{settingsForm.brandColor}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-amber-50 border-l-4 border-amber-500 p-3 rounded-lg flex items-start gap-3 mb-4">
                                ...
                                <p className="text-xs text-amber-800">
                                    Configure or update your SMTP server details for email delivery.
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                />
                                <label htmlFor="modalSmtpSecure" className="text-sm text-slate-700">
                    ...
                    {settingsTab === 'data_format' && (
                        <div className="space-y-4">
                            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                <label className="block text-xs font-medium text-slate-700 mb-1">Title Rows to Skip</label>
                                <p className="text-xs text-slate-500 mb-2">
                                    Number of rows to skip at the top of the Excel file before the header row starts.
                                    (e.g., if headers are on Row 3, skip 2 rows).
                                </p>
                                <div className="flex items-end gap-4">
                                    <input
                                        ...
                                        onChange={(e) => setSettingsForm({ ...settingsForm, titleRows: parseInt(e.target.value) || 0 })}
                                ...
                                <div className="min-w-[600px]">
                                    ...
                                    <div className="max-h-[400px] overflow-y-auto">
                                        {settingsForm.columnMapping.map((col, index) => (
                                            <div key={col.id} className="grid grid-cols-12 gap-2 p-3 border-b border-slate-100 items-center hover:bg-slate-50">
                                                {/* Order / Index */}
                                                ...
                                                <div className="col-span-3">
                                                    <CustomSelect
                                                        ...
                                                        onChange={(val) => updateColumn(index, 'systemField', val)}
                                                        placeholder="-- Ignore --"
                                                        options={SYSTEM_FIELDS.map(field => {
                                                            // Check if mapped by anyone (including self, to show status in list)
                                                            const isMapped = settingsForm.columnMapping.some(c => c.systemField === field.value);
                                                            ...
                                                            const isMappedByOther = settingsForm.columnMapping.some(c => c.systemField === field.value && c.id !== col.id);
                                                ...
                                                <div className="col-span-2 flex justify-center">
                                                    <input
                                                        ...
                                                        onChange={(e) => updateColumn(index, 'showOnDashboard', e.target.checked)}
                        ...
                        <div className="space-y-6">
                            <div className="p-4 bg-blue-50 border border-blue-100 rounded-lg">
                                <p className="text-sm text-blue-800 mb-3">
                                    Need to change your password? We can send you a secure link to reset it.
                                </p>
                                <button
                                    onClick={async () => {
                            ...
                            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                                <h3 className="text-sm font-bold text-red-900 mb-2">Delete Company Profile</h3>
                                <p className="text-sm text-red-800 mb-3">
                                    Permanently delete your company profile and all associated data. This action cannot be undone and requires OTP verification.
                                </p>
                                <button
                                    onClick={() => {
                        ...
                        {settingsTab === 'data_format' && (
                            <>
                                <label className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-300 hover:bg-purple-50 hover:text-purple-600 hover:border-purple-200 text-slate-700 text-sm font-medium rounded-lg cursor-pointer transition shadow-sm order-1">
                                    <Upload size={16} />
                                    Upload Sample
                                    <input type="file" accept=".xlsx, .xls" onChange={handleSampleUpload} className="hidden" />
                                </label>
                                <button

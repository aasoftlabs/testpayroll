...
import { X, User, Briefcase, Calculator } from 'lucide-react';
...
export default function EmployeeDetailsModal({ employee, onClose }) {
    ...
    const getCategory = (key) => {
        ...
        if (
            ['basic', 'other alw', 'dist alw', 'da', 'net pay', 'net salary'].includes(lowerKey) || // Hide logic handled below or explicit hide
        ...
        // Personal Details
        if (['name', 'dob', 'mobile', 'email', 'location', 'pan'].includes(lowerKey)) {
            return 'personal';
        }
        
        // Employment Details
        if (['emp code', 'designation', 'department', 'doj', 'days paid', 'uan no', 'esic no', 'pf no'].includes(lowerKey)) {
            return 'employment';
        }
    ...
    const personalDetails = entries.filter(([key]) => getCategory(key) === 'personal');
    const employmentDetails = entries.filter(([key]) => getCategory(key) === 'employment');
    const bankDetails = entries.filter(([key]) => getCategory(key) === 'bank');
    const financialDetails = entries.filter(([key]) => getCategory(key) === 'financial');
    const netPayDetails = entries.filter(([key]) => getCategory(key) === 'net_pay_custom');
    
    // Reorder: Ensure Arrears comes before Gross Salary
    const grossIndex = financialDetails.findIndex(([key]) => key.toLowerCase() === 'gross salary');
    const arrearsIndex = financialDetails.findIndex(([key]) => key.toLowerCase() === 'arrears');
    ...
    const summaryDetails = entries.filter(([key]) => getCategory(key) === 'summary');
    ...
    const DetailRow = ({ label, value, highlight = false }) => {
        ...
        const shouldHighlight = highlight || ['gross salary', 'ttl ded'].includes(lowerLabel);
        
        // Handle Date formatting for DOB and DOJ
        if ((lowerLabel === 'dob' || lowerLabel === 'doj') && typeof value === 'number') {
        ...
        return (
            <div className={`flex justify-between items-center py-2 border-b border-slate-50 last:border-0 ${shouldHighlight ? 'bg-indigo-50 -mx-2 px-2 rounded-lg' : ''}`}>
                <span className="text-sm text-slate-500 font-medium capitalize">{formatLabel(label)}</span>
                <span className={`text-sm font-semibold ${shouldHighlight ? 'text-indigo-600' : 'text-slate-800'}`}>
                    {displayValue}
                </span>
            </div>
        );
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                                ...
                                {summaryDetails.map(([key, value]) => (
                                    <DetailRow key={key} label={key} value={value} highlight={true} />
                                ))}
                                ...
                                {netPayDetails.length > 0 && (() => {
                                    ...
                                    return (
                                        <div key={key} className={`mt-4 p-4 rounded-xl border-2 ${employee['__mismatch'] ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-100'}`}>
                                            <div className="flex justify-between items-center mb-1">
                                                <span className={`text-sm font-bold ${employee['__mismatch'] ? 'text-amber-800' : 'text-emerald-800'}`}>
                                                    {formatLabel(key)}
                                                </span>
                                                <span className={`text-xl font-bold ${employee['__mismatch'] ? 'text-amber-700' : 'text-emerald-700'}`}>
                                            ...
                                            {employee['__mismatch'] && (
                                                <div className="mt-2 pt-2 border-t border-amber-200 text-xs text-amber-800 flex items-start gap-2">
                                                    ...
                                                    <div>
                                                        <span className="font-semibold block">Calculation Mismatch detected!</span>
                                                        System calculated: <span className="font-mono">{formatCurrency(employee['__calculatedNet'])}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </div>

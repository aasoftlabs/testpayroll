  ...
  return (
    <div className="space-y-6 mb-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-6">
        <div>
          <h2 className="text-2xl font-bold text-(--brand-color)">
            Payroll Dashboard
          </h2>
          <div className="flex items-center gap-3 mt-1">
            ...
            {payrollMonth && (
              <div
                className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${isPeriodFromExcel ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-slate-100 text-slate-600 border border-slate-200"}`}
              >
                <span>
                  {payrollMonth} {payrollYear}
                </span>
                <span className="opacity-70 text-[10px] uppercase tracking-wide">
                  ({isPeriodFromExcel ? "From Excel" : "Auto"})
                </span>
              </div>
            )}
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 font-medium rounded-lg transition-all shadow-sm cursor-pointer text-sm active:scale-[0.98] ${
              isSending || employees.length === 0
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="bg-white p-3 rounded-xl border border-slate-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex flex-col gap-3 hover:border-slate-300 transition-colors"
          >
      ...
      {isSending && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100 animate-in fade-in slide-in-from-top-2">
          ...
          <p className="text-xs text-slate-500 mt-2 text-center font-medium">
            Sent {sendingProgress.current} of {sendingProgress.total} emails
          </p>
        </div>
      )}
    </div>

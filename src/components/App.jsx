...
export default function App() {
    ...
    const { user, login, signup, logout, loading, verifyEmail, resetPassword } = useAuth();
    ...
    useEffect(() => {
        if (companyProfile?.brandColor) {
            document.documentElement.style.setProperty('--brand-color', companyProfile.brandColor);
        } else {
    ...
    const handleUpdateProfile = async (updatedSettings) => {
        try {
            ...
            const updatedProfile = {
                ...
                smtpSecure: updatedSettings.smtpSecure,
                columnMapping: updatedSettings.columnMapping || companyProfile.columnMapping,
                titleRows: updatedSettings.titleRows || 0
            };
            await setDoc(doc(db, 'companies', 'main-profile'), updatedProfile, { merge: true });
    ...
    const handleLogin = async () => {
        try {
            if (!companyProfile?.hrEmail) {
                setErrorMessage('System Error: No admin email found. Please contact support or reset.');
                return;
            }
            ...
            const processedData = dataRows.map((row) => {
                ...
                const getVal = (key) => {
                    ...
                    if (companyProfile?.columnMapping) {
                        const col = companyProfile.columnMapping.find(c => c.systemField === key);
                        if (col) {
                ...
                let deductions = pf + pt + esi + tds + vpf + staffLoan + salAdv + lwf + mobileDed + otherDed + medicalIns;
                ...
                return mappedEmp;
            }).filter(emp => emp && Object.keys(emp).length > 0 && (emp['Net Pay'] !== undefined || emp['Name'] || emp['Emp Code'])); // Basic filter for empty rows

            setEmployees(processedData);
            showToast(`Successfully imported ${processedData.length} employees`, 'success');
        };
    ...
    const handleReset = () => {
        setConfirmModal({
            ...
            message: 'Are you sure you want to reset all payroll data? This will remove all imported employees and cannot be undone.',
            type: 'danger',
            onConfirm: () => {
                setEmployees([]);
    ...
    const handleDownloadSingle = (employee) => {
        ...
        doc.save(`Payslip_${employee.Name || 'Employee'}_${month}_${year}.pdf`);
    };
    ...
    const executeSendPayslips = async () => {
        ...
        for (let i = 0; i < targetIndices.length; i++) {
            ...
            try {
                const pdfBase64 = generatePDFBase64(employee, month, year, companyProfile);
                ...
                if (!response.ok) {
                    ...
                    showToast(`Failed to send email to ${employee.Email}: ` + (errorData.details || errorData.error), 'error');
                }
            } catch (error) {
                showToast(`Error sending to ${employee.Name}: ` + error.message, 'error');
            }
        }
    ...
    const sendPayslips = () => {
        const count = selectedEmployees.length > 0 ? selectedEmployees.length : employees.length;
        if (count === 0) return;
    ...
    const handleDownloadTemplate = () => {
        ...
        if (companyProfile?.columnMapping && companyProfile.columnMapping.length > 0) {
            ...
            headers = [
                'S.No.', 'EMP CODE', 'EMPLOYEE NAME', 'ACCOUNT NUMBER', 'BANK NAME', 'BRANCH NAME', 'IFSC CODE',
                'DESIGNATION', 'DEPARTMENT', 'DOJ', 'LOCATION', 'EMAIL', 'ESI IP NO.', 'PF NO.', 'UAN NO.',
                'BASIC', 'DA', 'HRA', 'CONV \n ALW', 'MED \n ALW', 'OTHER\n ALW', 'DIST\n ALW', 'ARREARS',
                'GROSS\n SALARY', 'PRESENT \n DAYS'
            ];

            row1 = {
                'S.No.': '1', 'EMP CODE': 'EMP001', 'EMPLOYEE NAME': 'John Doe', 'ACCOUNT NUMBER': '1234567890',
                'BANK NAME': 'Test Bank', 'BRANCH NAME': 'Main', 'IFSC CODE': 'BANK001',
                'DESIGNATION': 'Dev', 'DEPARTMENT': 'IT', 'DOJ': '12/01/2026', 'LOCATION': 'HQ', 'EMAIL': 'john@example.com',
                'ESI IP NO.': 'ESIC001', 'PF NO.': 'PF001', 'UAN NO.': 'UAN001',
                'BASIC': 50000, 'DA': 2000, 'HRA': 20000, 'CONV \n ALW': 5000, 'MED \n ALW': 2000,
                'OTHER\n ALW': 3000, 'DIST\n ALW': 1000, 'ARREARS': 500,
                'GROSS\n SALARY': 80000, 'PRESENT \n DAYS': 30
            };
        }
    ...
    return (
        <div className="min-h-screen bg-slate-100 font-sans text-slate-900 flex flex-col">
                            ...
                            <div className="relative">
                                ...
                                >
                                    <div
                                        ...
                                        style={{
                                            backgroundColor: 'color-mix(in srgb, var(--brand-color), white 90%)',
                                            borderColor: 'color-mix(in srgb, var(--brand-color), white 80%)',
                                    ...
                                    <div className="hidden md:block text-left mr-1">
                                        <p className="font-semibold leading-none" style={{ color: 'var(--brand-color)' }}>{companyProfile?.userName || 'Admin'}</p>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr
                                            ...
                                            style={{ backgroundColor: 'color-mix(in srgb, var(--brand-color), white 95%)' }}
                                        >
                                            <th className="px-4 py-3 w-12 text-center">
                                                ...
                                                >
                                                    {selectedEmployees.length === employees.length && employees.length > 0 ? (
                                                        <CheckSquare size={18} className="text-(--brand-color)" />
                                                    ) : (
                                                        <Square size={18} />
                                                    )}
                                                </button>
                                            ...
                                            {/* Dynamic Columns */}
                                            {companyProfile?.columnMapping?.filter(col => col.showOnDashboard).map((col) => {
                                                const isEssential = col.systemField === 'Emp Code' || col.systemField === 'Name';

                                                let responsiveClass = 'hidden md:table-cell';
                                                if (col.systemField === 'Email') responsiveClass = 'hidden 2xl:table-cell';
                                                else if (col.systemField === 'Location') responsiveClass = 'hidden xl:table-cell';
                                                else if (col.systemField === 'Department') responsiveClass = 'hidden lg:table-cell';

                                                return (
                                                    <th key={col.id} className={`px-4 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wider whitespace-nowrap ${isEssential ? '' : responsiveClass}`}>
                                                        {col.header}
                                                    </th>
                                                );
                                            })}
                                            <th className="px-4 py-3 text-xs font-semibold text-slate-700 text-right uppercase tracking-wider">Net Pay</th>
                                            <th className="hidden md:table-cell px-4 py-3 text-xs font-semibold text-slate-700 text-right uppercase tracking-wider w-16">Actions</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {employees.map((emp, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {companyProfile?.columnMapping?.filter(col => col.showOnDashboard).map((col) => {
                                                        let val = '';
                                                        if (col.systemField && emp[col.systemField] !== undefined) {
                                                        ...
                                                        return (
                                                            <td key={col.id} className={`px-4 py-3 text-sm text-slate-700 whitespace-nowrap ${isEssential ? '' : responsiveClass}`}>
                                                                {val}
                                                            </td>
                                                        );
                                                    })}
                                                    <td className={`px-4 py-3 text-sm font-semibold text-right ${emp['__mismatch'] ? 'text-amber-600' : 'text-emerald-600'}`}>
                                                            ...
                                                            {emp['__mismatch'] && (
                                                                <div className="group relative">
                                                                    ...
                                                                    <div className="absolute bottom-full right-0 mb-2 hidden group-hover:block w-48 p-2 bg-slate-800 text-white text-xs rounded shadow-lg z-50 whitespace-normal text-left">
                                                                        Mismatch! System Calculated: {formatCurrency(emp['__calculatedNet'])}
                                                                    </div>
                                                                </div>
                                                            )}
                                                    ...
                                                    <td className="hidden md:flex px-4 py-3 items-center justify-center gap-2">
                                                        ...
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleDownloadSingle(emp); }}
            ...
            <Footer companyName={companyProfile?.name} brandColor={companyProfile?.brandColor} />
